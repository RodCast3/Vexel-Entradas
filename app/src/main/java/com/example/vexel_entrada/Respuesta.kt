...
class Respuesta : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when(intent.getStringExtra("modo")){
            "foto" -> {
                ...
                if (ruta != null) {
                    enviarFoto(File(ruta),"entrada")
                }
            }
            "qr" -> {
                val palabra = intent.getStringExtra("palabra")
                if (palabra != null) {
                    enviarPalabra(palabra, "entrada")
                }
            }
        }
    ...
    private fun enviarFoto(foto: File, tipoRegistro: String) {
        ...
        val requestBody = MultipartBody.Builder()
            ...
            .addFormDataPart("tipo_registro", tipoRegistro)  // <-- Aquí agregamos el tipo de acceso
            .build()
            ...
            override fun onResponse(call: Call, response: Response) {
                runOnUiThread {
                    if (response.code == 200) {
                        ...
                        // OJO: ahora el backend responde con "razon" cuando hay error
                        val json = JSONObject(response.body?.string())
